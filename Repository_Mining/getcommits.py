...
def ParseLogSmp(LogFile,repo):
    ...
    commits_list.to_csv("./pyc_commit.csv", mode='a', header=False, index=False, sep=',')
...
repo_info = pd.read_csv('./pyc_repo_list.csv',sep=',')
repo_list = repo_info['full_name']


for i in tqdm(range(len(repo_list))): #
    print("project number",i)
    repo = repo_list[i]
