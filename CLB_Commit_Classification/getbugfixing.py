...
repo_info = pd.read_csv('./pyc_commit.csv',sep=',',header=None,names=['sha','repo_fullname', 'issune_num'])
...
for i in tqdm(range(len(sha_list))):
    ...
    found_bug = False
    print(issue_list[i])
    for issue_No in str(issue_list[i]).split(','):
        if found_bug:
            break
    ...
    commits_list = pd.DataFrame({'sha': new_sha,
                             'repo': new_repo,
                             'issue':issue_tag})
    commits_list.to_csv("pyc_bug_commit.csv", mode='a', header=False, index=False, sep=',')
    print(f"{i} commit saved")
